...
class TestGoogleSheetsService:
    ...
    def test_read_sheet_with_retry_on_rate_limit(self, sheets_service, mock_retry_handler):
    ...
    def test_read_sheet_api_error(self, sheets_service, mock_sheets_client):
        ...
        mock_sheets_client.spreadsheets().values().get().execute.side_effect = HttpError(
    ...
    def test_write_sheet_success(self, sheets_service, mock_sheets_client):
        ...
        mock_sheets_client.spreadsheets().values().update().execute.return_value = mock_response
    ...
    def test_write_sheet_with_headers(self, sheets_service, mock_sheets_client):
        ...
        sheets_service.write_sheet('test-sheet-id', 'Sheet1!A1', test_data, include_headers=True)
    ...
    def test_batch_read_sheets(self, sheets_service, mock_sheets_client):
        ...
        mock_sheets_client.spreadsheets().values().batchGet().execute.return_value = mock_response
    ...
    def test_clear_sheet_range(self, sheets_service, mock_sheets_client):
        ...
        mock_sheets_client.spreadsheets().values().clear().execute.return_value = mock_response
    ...
    def test_create_sheet(self, sheets_service, mock_sheets_client):
        ...
        mock_response = {
            'replies': [{'addSheet': {'properties': {'title': 'NewSheet', 'sheetId': 123}}}]
        }
        mock_sheets_client.spreadsheets().batchUpdate().execute.return_value = mock_response
...
class TestGoogleSheetsServiceIntegration:
    ...
    def test_real_api_connection(self, test_config):
        ...
        df = service.read_sheet(
            test_config.project_terms_file_id,
            'Sheet1!A1:Z100'
        )
...
class TestGoogleSheetsServiceErrorHandling:
    ...
    def test_invalid_range_handling(self, sheets_service, mock_sheets_client):
        ...
        mock_sheets_client.spreadsheets().values().get().execute.side_effect = invalid_range_error
